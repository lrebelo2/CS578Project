...
CodeFlower.prototype.update = function(json) {
  ...
  rootInd = nodes[(total-1)].id;
    ...
    .append("p")
      .text("Total number of nodes is " + total);
  ...
  this.node.transition()
    .attr("r", function(d) { 
      ...
      if(d.id ==  rootInd) 
        a = 10;
      return d.children ? a : Math.pow(d.size, 2/5) || 1; })
    .style("fill", function color(d) {
      ...
      return "hsl(" + parseInt(360 / total * d.id, 10) + ",90%," + a + "%)";
    });
...
CodeFlower.prototype.mouseover = function(d) {
  var txt = d.name + ": " + d.size + " loc";
  if(d.name.endsWith(".class")){
    txt = txt + "   dependents: " + d.dependents + ", dependings: " + d.dependings;
  }
